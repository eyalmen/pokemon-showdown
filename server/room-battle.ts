...
export class RoomBattle extends RoomGames.RoomGame {
	...
	async logBattle(
		...
		const filePath = `${logpath}${this.room.getReplayData().id}.log.json`;
		this.processNewLog(filePath);
	}

	properHTMLFile(file: any, p1: any, p2: any, filename: any, format: any, uploaded: any, log: any) {
		let beginHTML = `<!DOCTYPE html>
		<html><head>
		
			<meta charset="utf-8" />
		
			<title>${format} replay: ${p1} vs. ${p2} - Pok&eacute;mon Showdown</title>
			...
			<div class="pfx-panel"><div class="pfx-body" style="max-width:1180px">
				<div class="wrapper replay-wrapper">
					...
					<pre class="urlbox" style="word-wrap: break-word;">https://replay.pseudo.gq/${filename}</pre>
		
					<h1 style="font-weight:normal;text-align:left"><strong>${format}</strong>: <a href="https://pokemonshowdown.com/users/${p1}" class="subtle">${p1}</a> vs. <a href="https://pokemonshowdown.com/users/${p2}" class="subtle">${p2}</a></h1>
					<p style="padding:0 1em;margin-top:0">
						<small class="uploaddate" data-timestamp="1390960565"><em>Uploaded:</em> ${uploaded}</small>
					</p>
		
					<div id="loopcount"></div>
				</div>
		
				<input type="hidden" name="replayid" value="${filename}" />
		...
		<a href="/" class="pfx-backbutton" data-target="back"><i class="fa fa-chevron-left"></i> Other replays</a>
	...
	<script src="/js/replay.js?1e09ceb9"></script>
		...
		file.write(beginHTML);
		file.write(log);
		file.write(endHTML);
		file.close();
	}

	logToReplay(logJSON: JSON, location: string) {
		
	}
	getBattleLog(battleLog: string): string {
		battleLog = battleLog.split('"log":["')[1];
		battleLog = battleLog.split('"],"')[0];
		battleLog = battleLog.replace('","', "\n");
		return battleLog;
	}
	
	addBackSlashes(battleLog: string): string {
		for (let i = 0; i < battleLog.length; i++) {
			if (battleLog[i-2] != "/" && battleLog[i-1] != "\\") {
				if (battleLog[i] == "/") {
					if (battleLog[i-1] != " " && battleLog[i+1] != "s") {
						battleLog = battleLog.slice(0, i) + "\\" + battleLog.slice(i);
					}
				}
			}
		}
	...
	processNewLog(info: string) {
		const file = fs.openSync(info, 'r');
		var log = fs.readFileSync(file).toString();
		let p1 = log.split('"p1":"')[1];
		p1 = p1.split('","p2')[0];
		let p2 = log.split('"p2":"')[1];
		p2 = p2.split('","p1team')[0];
		log = this.getBattleLog(log);
		log = this.addBackSlashes(log);
		...
		let battle = '';
		let x = '';
		var fileNew = undefined;
		// var heheFile = undefined;
		if (pathName.includes('nationaldex')) {
			return;
		}
		if (pathName.includes('gen8pseudos-')) {
			const fileName = pathName.split("gen8pseudos-")[1];
	
			battle = `gen8pseudos-${fileName.split(".log.json")[0]}`;
			fileNew = fs.openSync(`${battle}.html`, 'w+');
			// heheFile = fs.openSync(`pokemon-showdown-client/replays/replays/${battle}.html`, 'w+');
			format = "[Gen 8] Pseudos";
		} else if (pathName.includes('gen8pseudossuspect-')) {
			const fileName = pathName.split("gen8pseudossuspect-")[1];
			battle = `gen8pseudossuspect-${fileName.split(".log.json")[0]}`;
			fileNew = fs.openSync(`${battle}.html`, 'w+');
			// heheFile = fs.openSync(`pokemon-showdown-client/replays/replays/${battle}.html`, 'w+');
			format = "[Gen 8] Pseudos Suspect";
		} else {
			return;
		}
		
		const fileBirthTime = String(fs.statSync(pathName).birthtime);
		const lastModified = fileBirthTime.replace("T", " ").replace("Z", " UTC");
		const title = `<title>${format} replay: ${p1} vs. ${p2}</title>`;
		const meta = `<meta name="description" content="Watch a replay of a Pokémon battle between ${p1} and ${p2} (${format})"/>`; 
		...
		<body>    
			...
			<input type="hidden" name="replayid" value="unregisteredserver-${battle}" />
			<div class="battle"></div><div class="battle-log"></div><div class="replay-controls"></div><div class="replay-controls-2"></div></div>
			<script type="text/plain" class="battle-log-data">`;
	
		const endHTML = `</script>
		<script>
		let daily = Math.floor(Date.now()/1000/60/60/24);document.write('<script src="../pokemon-showdown-client/js/replay-embed.js?version'+daily+'"></'+'script>');
		</script>
		</body>
		</html>`;
		
		fs.writeSync(fileNew, beginHTML);
		fs.writeSync(fileNew, log);
		fs.writeSync(fileNew, endHTML);
		fs.closeSync(fileNew);
	
		// this.properHTMLFile(heheFile, p1, p2, battle, format, lastModified, log);
	}
